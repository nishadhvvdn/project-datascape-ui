...
(function (ACWF, undefined) {
	...
	ACWF.defaultOptions = {
		...
		// colors from 
		phaseStyles: [
			{ name: "1", style: { color: "#000000" } },
			{ name: "2", style: { color: "#f00000" } },
			{ name: "3", style: { color: "#2556ca" } },
	...
	// custom : included anglex parameter.
	ACWF.WaveformAnalyzer = function (samples, label, angleFromData) {
		samples = samples || [0];
		label = label || "";
		var rms = 0;
		...
		(function () {
			...
			var calculateAngle = angleFromData * 0.01745329251;
			phasor = new ACWF.Phasor(rms, calculateAngle, label);
	...
	ACWF.Phasor.prototype.toString = function () {
		//	console.log("Phasor " + this.label + ": " + this.magnitude + " @ " + rad2deg(this.angle) + " deg");
	...
	ACWF.WaveformSet = function (_samplesPerCycle) {
		...
		this.addWaveform = function (waveform, unit, phase, angle) {
			// add some properties for later use
			waveform.unit = unit;
			waveform.phase = phase;
			// custom : added new property waveform.angle in waveform object.
			waveform.angle = angle;
		...
		this.analyze = function (firstSample) {
			...
			waveforms.forEach(function (waveform) {
				...
				// custom : included waveform.angle parameter.
				waveform.analysis = new ACWF.WaveformAnalyzer(samplesToAnalyze, waveform.label, waveform.angle);
			});
			return this;
		};
	...
	ACWF.WaveformSet.create = function (waveformData, options) {
		...
		for (var i = 0; i < waveformData.data.length; i++) {
			...
			waveformSet.addWaveform(new ACWF.Waveform(data.samples, data.label), data.unit, data.phase, data.angle);
		}
