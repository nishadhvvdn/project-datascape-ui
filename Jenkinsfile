...
pipeline{
    ...
    triggers {
        cron(env.BRANCH_NAME == 'develop' ? '00 17 * * *' : '')
    }
    ...
    parameters {
       choice(name: 'ENVIRONMENT_NAME', choices: ['DEV', 'QA', 'QA-LOAD-TEST', 'PROD', 'KCEC'], description: 'Select the environment to be deployed')
    }
    ...
    stages{
        ...
        stage('Build-Code'){
            steps{
		     ...
		     else if("${params.ENVIRONMENT_NAME}" == "KCEC")
                    {
                        uiBuild()
                    }
                    else
    ...
    post{
        success{
            script{
		    badge("${BUILD_USER}")
		    build job: 'project-datascape-ui_CD',
